...
class CreateProcedureTransactionPenjualan extends Migration
{
    /**
     * Run the migrations.
     *
...
BEGIN
    ...
    read_loop: LOOP
        ...
        -- Mendapatkan data produk termasuk stok dan harga
        SELECT nama_produk, harga_jual, stok INTO nama_produk, harga, stok
        FROM produk
        WHERE kode_produk = kode_produk;

        -- Validasi jika stok tidak mencukupi
        IF stok < jumlah THEN
            SET error_message = CONCAT('Stok tidak mencukupi untuk produk: ', kode_produk);
            ROLLBACK;
        ...
        INSERT INTO penjualan_detail (nomor_invoice, nama_produk, jumlah, harga_jual, subtotal, created_at, updated_at)
        VALUES (nomor_invoice, nama_produk, jumlah, harga, harga * jumlah, NOW(), NOW());
    END LOOP;
